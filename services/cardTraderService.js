...
exports.fetchOrders = async () => {
    let orders = [], page = 1, limit = 100;
    const fromDate = formatDate(new Date(Date.now() - 2 * 24 * 60 * 60 * 1000));

    while (true) {
        const { data } = await axios.get('https://api.cardtrader.com/api/v2/orders', {
            headers,
...
exports.fetchOrderDetails = async (orderId) => {
    const { data } = await axios.get(`https://api.cardtrader.com/api/v2/orders/${orderId}`, { headers });
    return data;
};

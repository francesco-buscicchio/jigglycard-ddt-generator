...
exports.processOrdersFromCSV = async () => {
    ...
    for (const order of ordersData) {
        ...
        if (isNaN(datePaid.getTime()) || (currentDate - datePaid) > (20 * 24 * 60 * 60 * 1000)) continue;
        ...
        //to do: mappare i country con il nome delle nazioni
        const docAddress = new Address({
            name: order.shippingAddressName.trim(),
            street: `${order.shippingAddressStreet}${order.shippingAddressExtra ? ', ' + order.shippingAddressExtra : ''}`.trim(),
            zip: order.shippingAddressZip.trim(),
            city: order.shippingAddressCity.trim(),
            state_or_province: '',
            country: order.shippingAddressCountry.trim(),
            orderCode: orderId
        });
        ...
        const shippingMethod = new ShippingMethod({
            ...
            price: (convertPriceToCents(order.totalValue) - convertPriceToCents(order.articleValue)) / 100
        });

        const shippingItems = articlesData
            .filter(item => item[`"idOrder"`] === orderId)
            .map(item => new ShippingItem({
                name: item.name.trim(),
